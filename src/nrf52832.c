...
NRF52832_t *nrf52832_new(const uint8_t *program, size_t program_size, size_t sram_size)
{
    ...
    NEW_PERIPH(chip, SPIM, spim, spim[0], x(4000, 3000), 0x1000, 0x03, chip->spi);
    NEW_PERIPH(chip, TWIM, twim, twim[0], x(4000, 3000), 0x1000, 0x03, chip->i2c);
    NEW_PERIPH(chip, SPIM, spim, spim[1], x(4000, 4000), 0x1000, 0x04, chip->spi);
    NEW_PERIPH(chip, TWIM, twim, twim[1], x(4000, 4000), 0x1000, 0x04, chip->i2c);
    NEW_PERIPH(chip, GPIOTE, gpiote, gpiote, x(4000, 6000), 0x1000);
    NEW_PERIPH(chip, SAADC, saadc, saadc, x(4000, 7000), 0x1000, chip->pins);
    NEW_PERIPH(chip, TIMER, timer, timer[0], x(4000, 8000), 0x1000, 0x08, 4);
    NEW_PERIPH(chip, TIMER, timer, timer[1], x(4000, 9000), 0x1000, 0x09, 4);
    NEW_PERIPH(chip, TIMER, timer, timer[2], x(4000, A000), 0x1000, 0x0A, 4);
    ...
    NEW_PERIPH(chip, TIMER, timer, timer[3], x(4001, A000), 0x1000, 0x1A, 6);
    NEW_PERIPH(chip, TIMER, timer, timer[4], x(4001, B000), 0x1000, 0x1B, 6);
    NEW_PERIPH(chip, SPIM, spim, spim[2], x(4002, 3000), 0x1000, 0x23, chip->spi);
