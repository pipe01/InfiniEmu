...
#define OP_IGNORE_LOAD_DATA      \
    if ((op) == OP_LOAD_DATA)    \
    {                            \
        return MEMREG_RESULT_OK; \
    }

#define OP_ASSERT_SIZE(op, size)                           \
    if ((op) != OP_READ_##size && (op) != OP_WRITE_##size) \
    {                                                      \
        OP_INVALID_SIZE;                                   \
    }
#define OP_ASSERT_READ(op) \
    if ((op) < 0)          \
    {                      \
        OP_INVALID_ACCESS; \
    }
#define OP_ASSERT_WRITE(op) \
    if ((op) > 0)           \
    {                       \
        OP_INVALID_ACCESS;  \
    }
...
{
    memory_map_do_operation(map, addr, (memreg_op_t)-size, &value);
}
